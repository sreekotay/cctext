...
| Workbook | `d.derived` → `RtxWb` (`core/wb.ccs`, `*.wb.md` only): own heap arenas for the model (names, cells, ASTs — one per column for row formulas like `=@a * @b` —, graph, maintained-aggregate partials and leaves), values, and per-operation scratch; the node array (160 B a cell), a recalc's queued row deltas and old values, and cycle values in heap vectors | a structural edit (header, fence, caption, a table's end) or garbage past its bound rebuilds the model; cell / calc-line / prose edits and whole-row inserts / deletes inside a table patch it (rows as MA deltas, docs/workbook.md "W2 as built"); in a workbook of 1 MiB or more the keystroke path (derived hooks, status) does not rebuild: values hide and the host's idle loop rebuilds once typing pauses (`rtx_wb_idle`, docs/workbook.md "W2 at scale"); `RtxDoc.destroy()` |
