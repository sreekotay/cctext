...
- Edits are replaces on a history stack (coalesced typing; large deletes keep piece refs so undo/redo can splice them back). Save writes pieces to a sibling temp file, preserves mode, fsyncs (file + best-effort dir), then renames. `--backup` writes through the path (keeps symlink / inode) and copies only the dirty span to `path~` (RTXB). Dirty is `hist.head != saved_head`.
