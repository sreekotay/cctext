...
- Layout: `measure(run, bytes) -> width` and `line_height(run) -> height`. The GUI supplies glyph advances via libui text layout (Core Text on macOS, Pango on Linux); `cctext` supplies `wcwidth`.
...
- `cctext-ui` — libui-ng window (macOS: AppKit; Linux: GTK 3). Paint runs in the libui area Draw callback; text is libui's attributed-string layout.
