...
- **Deep search.** Typing a fragment in the file browser walks the tree (pumped, parallel; chrome shows `> Flattened search`). `>` is the same, explicit. A fragment is case-insensitive; `*.txt` is a real glob and stays a local listing so you can still walk directories.
