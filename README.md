...
- **Byte jump.** `g 50%` snaps to the mid-file line with a local read (8 KiB). A planted pin at that 1/32 slot re-keys the gutter; otherwise it shows `+1`… / `-L` until the prefix meets the camera. Arrows and wheel stay on that camera — they do not `line_of` the prefix. Absolute `g L` pumps the prefix and shows `scanning... N%` on the jump field.
